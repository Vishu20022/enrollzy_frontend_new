            ...
            <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
                <div class="card-header bg-white border-bottom py-3">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            <h5 class="mb-0 fw-bold text-dark">Edit Testimonial</h5>
                            <small class="text-muted">Modify student or alumni feedback details</small>
                        </div>
                    </div>
                </div>
                ...
                <div class="card-body p-4">
                    <form action="{{ route('testimonials.update', $testimonial->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" class="form-control form-control-lg @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name', $testimonial->name) }}" required placeholder="e.g. Rahul Singh" style="border-radius: 8px; font-size: 0.95rem;">
                            @error('name')
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" class="form-control form-control-lg @error('role') is-invalid @enderror" id="role" name="role" value="{{ old('role', $testimonial->role) }}" placeholder="e.g. MBA Student" style="border-radius: 8px; font-size: 0.95rem;">
                            @error('role')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea class="form-control @error('content') is-invalid @enderror" id="testimonial_content" name="content" rows="5" required placeholder="What did they say?" style="width: 100% !important; margin: 0 !important; border-radius: 8px; font-size: 0.95rem; resize: vertical;">{{ old('content', $testimonial->content) }}</textarea>
                            @error('content')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                                ...
                                <div class="d-flex align-items-center gap-3">
                                    @if($testimonial->image)
                                        <div class="flex-shrink-0">
                                            <img src="{{ asset($testimonial->image) }}" alt="{{ $testimonial->name }}" class="rounded-circle border border-2 border-light shadow-sm" style="width: 55px; height: 55px; object-fit: cover;">
                                        </div>
                                    @endif
