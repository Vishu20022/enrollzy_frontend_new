                    ...
                    @foreach ($organisations as $org)
                        <div class="col-lg-2 col-md-3 col-6 text-center mb-4">
                            <a href="{{ $org->detail_url }}" 
                               class="text-decoration-none uni-card premium-org-card {{ $org->logo_url ? 'rounded-circle' : 'rounded-circle' }} mx-auto mb-3"
                                @if ($org->logo_url)
                                    style="background-image: url('{{ env('BACKEND_URL') . '/' . $org->logo_url }}'); background-size: cover; background-position: center; background-repeat: no-repeat; aspect-ratio: 1/1; display: block;"
                                @else
